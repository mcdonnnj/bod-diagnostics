...
def output_domain_results(domain, results):
    ...
    for key in ("Domain Supports HTTPS", "Domain Enforces HTTPS", "Domain Uses Strong HSTS"):
        if key in results:
            bod_failed = True
    ...
    if bod_failed:
        ...
        print("\t(Domain Supports HTTPS and Domain Enforces HTTPS and Domain Uses Strong HSTS)")
        print("\t or (Live")
        print("\t     and (Base Domain HSTS Preloaded")
        print("\t          or (not HTTPS Full Connection and HTTPS Client Auth Required)")
        print("\t         )")
        print("\t    }")
...
def parse_csv(csv_file, domains=None):
    ...
    for row in csv_reader:
        ...
        domain_uses_strong_hsts = row["Domain Uses Strong HSTS"] or domain_fallback_check
        domain_uses_weak_crypto = row["Domain Supports Weak Crypto"]

        results[row["Domain"].lower()] = {}
        if not (domain_uses_https and domain_enforces_https and domain_uses_strong_hsts):
            results[row["Domain"].lower()]["Live"] = row["Live"]
            results[row["Domain"].lower()]["Base Domain HSTS Preloaded"] = row["Base Domain HSTS Preloaded"]}

            if not domain_uses_https:
                results[row["Domain"].lower()]["Domain Supports HTTPS"] = row["Domain Supports HTTPS"]
            if not domain_enforces_https:
                results[row["Domain"].lower()]["Domain Enforces HTTPS"] = row["Domain Enforces HTTPS"]
            if not domain_uses_strong_hsts:
                results[row["Domain"].lower()]["Domain Uses Strong HSTS"] = row["Domain Uses Strong HSTS"]

        if domain_uses_weak_crypto:
            results[row["Domain"].lower()]["Domain Uses Weak Crypto"] = row["Web Hosts With Weak Crypto"]
